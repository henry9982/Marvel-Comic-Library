...
const ComicCard = ({comic}) => {
  ...
  useEffect(()=>{
    const checkIfAlreadyAdded = async()=>{
      ...
      const readLaterRef = collection(db,'readLaterComics')
      const querySnapshot = await getDocs(
        query(readLaterRef,where('userID','==',userID),where('id','==',comic.id))
      )
      const existingComic = querySnapshot.docs[0]
      if (existingComic) {
        setIsFavorite(true)
      }else{
        setIsFavorite(false)
  ...
  const handleStar = async()=>{
    ...
    if (isFavorite) {
      ...
      if (existingComic) {
        setIsFavorite(false)
        await deleteDoc(doc(readLaterRef,existingComic.id))
      }else{
        setIsFavorite(true)
        await addDoc(readLaterRef,{
          userID,
          id:comic.id,
          image: {
                      ...
                      <div className=' top-0 border-2 border-black border-l-0 flex justify-between flex-col  right-0 bg-[#333333]  w-3/6 h-[98%] max-[400px]:w-[51%] max-[400px]:pl-[2px] characterInfoBox  my-auto bottom-0  absolute'>
                        ...
                        <a href={detailUrl} target='_blank'  className=' flex justify-between absolute bottom-0 max-sm:-bottom-[1px] max-[400px]:-bottom-[1.10pt] -right-[7px] box-info-btn cursor-pointer w-[115px] max-sm:w-[105px]  max-sm:h-7 max-[470px]:w-[96px] max-[400px]:w-[81px] max-[400px]:h-6 bg-white items-center h-7 overflow-hidden '>
                          <div className='bg-[#EE7214]   h-full flex - justify-center items-center relative'>
                            <div className='text-[12px] max-sm:text-[11px] max-[400px]:text-[9px] max-sm:-mb-[2px] max-[470px]:-mb-[5px] max-[470px]:text-[10px] max-[470px]:-mt-1 max-[400px]:-mt-[3px] pl-[2px]  uppercase font-poopins z-10 text-white'>Read Comic</div>
                            <div className='absolute bg-[#EE7214] w-10 h-10 rounded-full -right-4 -top-6 max-[400px]:-top-6 max-[400px]:-right-3'></div>
                            <div className='absolute bg-[#EE7214] w-10 h-10 rounded-full -right-4 top-[13px] max-[400px]:-right-3 max-[400px]:top-[10px]'></div>
                          </div>
